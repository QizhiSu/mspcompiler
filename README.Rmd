...
2. Tick *Use Subset* and click *Define Subset* to set detail parameters:
    ...
    * Enter which spectra should be exported in *List of Spectra IDs*. 
    ...
    ![Settings inside *Define Subset* in Lib2NIST Programe](vignettes/Lib2NIST_define_subset.png){width=500px}
    ...
    * The total number of spectra that your NIST library have can be checked in the *MS Search*
    ...
    ![Screenshot of checking total number of spectra in the MS Search Program](vignettes/check_number_of_spectra_nist.png){width=500px}
...
4. Select the library in *Input Libraries or Text Files* and *Convert*.
    ![Configuration of Lib2NIST](vignettes/Lib2NIST_configuration.png){width=500px}

Once you have the \*.MSP file (normally hundreds megabytes) and the correspondent 
\*.MOL folder (hundreds thousands .MOL files inside the folder) exported, you 
...
In case you want to expand your library by adding publicly available libraries,
continue with the instructions for each library. Otherwise, jump to the "combine
all libraries" section to add RI values accordingly.
