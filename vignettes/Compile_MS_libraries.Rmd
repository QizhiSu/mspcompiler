...
vignette: >
  %\VignetteIndexEntry{Compile_MS_libraries}
...
into a single, up-to-date, and MS-DIAL friendly msp file. This tutorial will be
divided into two sections explaining the pipeline to process EI and MS2 libraries,
respectively. To understand each function in detail, please use the "help or ?", 
?read_lib

## EI libraries
### NIST EI library
NIST is the most commonly used **commercial** EI library. Once you have NIST library
installed, you can transformed it into a msp file by *Lib2NIST*. Normally 
*Lib2NIST* will be installed along with the NIST library installation. If not, 
you can download it from https://chemdata.nist.gov/dokuwiki/doku.php?id=chemdata:nist17.
Please use the following settings in *Lib2NIST*:

1. *Add Input Libraries/Files*. For Agilent users, the input file can be found
in, for example, "C:/database/NIST.L". 

2. Tick *Define Subset* and enter to set detail parameters:

    * Enter which spectra will be exported in *List of Spectra IDs*. The total 
    number of spectra that your NIST library have can be checked in the *MS Search*
    program (Options -> Libraries -> Add both *mainlib* and *relib*). Then you can
    see the number of spectra your NIST library have. (include screenshot)
...
The whoel process is time-consuming, so we suggests to use parallel computing.
...
Below are the code to process NIST EI library. You may need to change the path 
accordingly.
```{r}
# Read the msp file into R.
nist_ei <- read_lib("home/NIST.MSP", type = "EI")
# Combine all mol files into a single sdf file for subsequent structure retrieval.
combine_mol2sdf("home/NIST.MOL", "home/nist.sdf")
# Extract structure based on the sdf file exported before.
nist_ei_structure <- extract_structure("home/nist.sdf", "home/nist_structure.txt")
...
needed.
```{r}
riken_ei <- read_lib("home/GCMS DB-Public-KovatsRI-VS3.msp",
...
```{r}
mona_ei <- read_lib("home/MoNA-export-GC-MS_Spectra.msp", type = "EI")
...
formats. To let it be correctly parsed by mspcompiler, please download both
...
NIST library. 
```{r}
swgdrug_ei <- read_lib("home/SWGDRUG.MSP", type = "EI")
combine_mol2sdf("home/SWGDRUG.MOL", "home/swgdrug.sdf")
swgdrug_ei_structure <- extract_structure("home/swgdrug.sdf", "home/swgdrug_structure.txt")
...
single file, assign experimental RI retrieved from "ri.dat" and "USER.DBU" files
...
# "~/Programs/nist17/mssearch/nist_ri". Assuming you have copied these two files
# in the home folder, then you will have:
nist_ri <- extract_ri("/home/ri.dat", "/home/USER.DBU")
...
found in "~/Programs/nist17/mssearch/nist_msms". The exported msp file has both
positive and negative modes mixed in a singled file, so we have to separated 
them by the separate_polarity function.
```{r}
nist_ms2 <- read_lib("home/NIST_ms2.MSP", type = "MS2")
combine_mol2sdf("home/NIST_ms2.MOL", "home/nist_ms2.sdf")
nist_ms2_structure <- extract_structure("home/nist_ms2.sdf", "home/nist_ms2_structure.txt")
...
public MS/MS positive and negative, respectively. 
```{r}
riken_ms2_pos <- read_lib("home/MSMS-Public-Pos-VS15.msp")
riken_ms2_neg <- read_lib("home/MSMS-Public-Neg-VS15.msp")
```

### MoNA MS2 libraries
The MoNA MS libraries can be downloaded from https://mona.fiehnlab.ucdavis.edu/downloads.
Please download "LC-MS/MS Positive Mode" and "LC-MS/MS Negative Mode", respectively.
```{r}
mona_ms2_pos <- read_lib("home/MoNA-export-LC-MS-MS_Positive_Mode.msp")
# Reorganize the SMILES field.
mona_ms2_pos <- reorganize_mona(mona_ms2_pos)

mona_ms2_neg <- read_lib("home/MoNA-export-LC-MS-MS_Negitive_Mode.msp")
...
```{r}
gnps <- read_lib("home/ALL_GNPS.mgf", format = "mgf")
# Compute MF
...
plan(sequential)
```
**Note**: Firstly, the RIKEN, MoNA, and GNPS MS2 libraries might contains some 
...
should not affect its use. Secondly, it takes a long time to process the NIST 
libraries. For a given lab, it may have a particular version and may not change
frequently, while others might change after some time, in particular MoNA and 
GNPS, because they allow online submission of MS2 spectra. For this reason, 
we recommend that once you have NIST libraries organized, save them as .Rda file 
and reuse it the next time. Whereas download the latest MoNA and GNPS every time
you organize the libraries to leverage all these resources.
...
```{r}
in_house <- read_multilibs("home/in_house")
```
